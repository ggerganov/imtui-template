...
int main() {
    ...
    while (isRunning) {
        ...
        if (g_ui.isHoveringLeftPanelResize) {
            ...
            drawList->AddRect(ImVec2(g_ui.leftPanelW + 0.5, 0), ImVec2(g_ui.leftPanelW + 1.1, screen->ny), ImGui::GetColorU32(ImGuiCol_Border));
        }

        // main window
        {
            g_ui.mainWindowW = screen->nx - g_ui.leftPanelW - 2;
            if (g_ui.showThreadPanel) {
                g_ui.mainWindowW -= g_ui.threadPanelW;
            }
            g_ui.mainWindowH = screen->ny;
...

            {
...

                {
                    ...
                    {
                        const auto p0 = ImGui::GetCursorScreenPos();
                        ImGui::SetCursorScreenPos(ImVec2(p0.x + g_ui.mainWindowW - 11, p0.y));
        ...
        if (g_ui.isHoveringThreadPanelResize) {
            ...
            drawList->AddRect(ImVec2(g_ui.leftPanelW + g_ui.mainWindowW + 0.5, 0), ImVec2(g_ui.leftPanelW + g_ui.mainWindowW + 1.1, screen->ny), ImGui::GetColorU32(ImGuiCol_Border));
        }

        // thread panel
        if (g_ui.showThreadPanel) {
            g_ui.threadPanelH = screen->ny;
            ...
            if (g_ui.threadChannel >= 0) {
                ...
                ImGui::BeginChild("messages", ImVec2(g_ui.threadPanelW - 1, g_ui.messagesWindowH), true);
...

                {
                    ...
                    {
                        const auto p0 = ImGui::GetCursorScreenPos();
                        ImGui::SetCursorScreenPos(ImVec2(p0.x + g_ui.threadPanelW - 11, p0.y));
                        ImGui::PushStyleColor(ImGuiCol_Text,          g_ui.colors.inputSendFG);
                        ImGui::PushStyleColor(ImGuiCol_Button,        g_ui.colors.inputSendBG);
                        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, g_ui.colors.inputSendBGHovered);
                        ImGui::PushStyleColor(ImGuiCol_ButtonActive,  g_ui.colors.inputSendBGActive);
        ...
        if (g_ui.isResizingThreadPanel) {
            g_ui.threadPanelW = screen->nx - ImGui::GetIO().MousePos.x - 2;
        }
